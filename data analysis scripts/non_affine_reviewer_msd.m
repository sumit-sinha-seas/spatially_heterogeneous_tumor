...
for i=1:size(intersecting_label,1)
  %  i
    ...
    for j=1:size(time,2)
        total_displ=req_track_particle(j,:)-req_track_particle(1,:);
        r_project=dot(total_displ,req_track_particle(end,1:3)-com)/(norm(req_track_particle(end,1:3)-com));
        theta_proj=sqrt((norm(total_displ))^2-r_project^2);
        msd_every_cell(j,i)=(r_project)^2;
    end
    if(dis_com<30)
    ...
    if(dis_com>10 && dis_com<20)
     ...
     if(dis_com>100)
        msd_layer_extra(:,end+1)=msd_every_cell(:,i);
    end
