...
vanhove_interior_cell_radial_tang=zeros(2,1);

vanhove_boundary_cell=zeros(3,1);
vanhove_boundary_cell_radial_tang=zeros(2,1);
...
for i=1:size(intersecting_label,1)
   i
    ...
    for j=1:size(time,2)-del_t
        ...
        disp_del=[del_x,del_y,del_z];
        
        r_project=dot(disp_del,req_track_particle(end,1:3)-com)/(norm(req_track_particle(end,1:3)-com));
        theta_proj=sqrt((norm(disp_del))^2-r_project^2);
       ...
       % path_length(1,i)=path_length(1,i)+ del_x;
             ...
             % vanhove_every_cell(1,i)= displacement(1,i)/path_length(1,i);
               ...
               if(dis_com<30)
                   ...
                   vanhove_interior_cell_radial_tang(:,end+1)=[abs(r_project);abs(theta_proj)];
               end
               if(dis_com>60)
                   vanhove_boundary_cell(:,end+1)=[del_x;del_y;del_z];
                   vanhove_boundary_cell_radial_tang(:,end+1)=[abs(r_project);abs(theta_proj)];
               end
    end
...
vanhove_boundary_cell_radial_tang(:,1)=[];
vanhove_interior_cell_radial_tang(:,1)=[];
